...
def sentence1() -> Expr:
    ...
    sentence1 = conjoin([sym1, sym2, sym3])
    return sentence1
...
def sentence2() -> Expr:
    ...
    sentence2 = conjoin([sym1, sym2, sym3, sym4])
    return sentence2
...
def sentence3() -> Expr:
    ...
    D = PropSymbolExpr("PacmanBorn", time = 0)

    sym1 = B % ((A & ~C) | (~A & D))
    sym2 = ~(A & D)
    sym3 = D
    
    return conjoin(sym1, sym2, sym3)
...
def plTrueInverse(assignments: Dict[Expr, bool], inverse_statement: Expr) -> bool:
    ...
    return pl_true(inverse, model=assignments)
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    newList = [~i[0] | ~i[1] for i in litList]
    return conjoin(newList)
    "*** END YOUR CODE HERE ***"
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # prop symbol representing pacman position implies one possible cause
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    # if not None...
    if sensorModel:
        # append prop logic to pacphysics_sentences for sensorModel
        pacphysics_sentences.append(sensorModel(t, non_outer_wall_coords))
    
    if successorAxioms and t > 0:
        # append prop logic to pacphysics_sentences for successor logic
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # define world behavior at time 0 and 1
    ...
    # pacman actions at time 0 and 1
    KB.append(PropSymbolExpr(action0, time=0))
    KB.append(PropSymbolExpr(action1, time=1))

    # successor states defined
    KB.append(allLegalSuccessorAxioms(1, walls_grid, non_outer_wall_coords))

    # find two possible models based on previous lines
    sym1 = findModel(conjoin(KB + [PropSymbolExpr(pacman_str, x1, y1, time=1)]))
    sym2 = findModel(conjoin(KB + [~PropSymbolExpr(pacman_str, x1, y1, time=1)]))
...
def positionLogicPlan(problem) -> List:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # add PropSymbolExpr for init position
    KB.append(PropSymbolExpr(pacman_str, x0, y0, time=0))
    ...
    for t in range(50):
        ...
        print(t)
        # print timestep
        steps = [PropSymbolExpr(pacman_str, x, y, time=t) for x,y in non_wall_coords]
        KB.append(exactlyOne(steps))
        sym1 = findModel(conjoin(KB) & PropSymbolExpr(pacman_str, xg, yg, time=t))
        if sym1:
            return extractActionSequence(sym1, actions)
        bleach = [PropSymbolExpr(d, time=t) for d in DIRECTIONS]
        KB.append(exactlyOne(bleach))
        bleachList = [pacmanSuccessorAxiomSingle(x, y, t+1, walls_grid) for x,y in non_wall_coords]
        for l in bleachList:    
            KB.append(l)
...
def foodLogicPlan(problem) -> List:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    KB = []
    # init foodSuccessorAxiom lambda function
    foodSuccessorAxiom = lambda x, y, t: PropSymbolExpr(food_str, x, y, time=t+1) % (PropSymbolExpr(food_str, x, y, time=t) & ~PropSymbolExpr(pacman_str, x, y, time=t))
    # create list of propositions that there is food
    new = [PropSymbolExpr(food_str, x, y, time=0) for x,y, in food]
    for i in new:
        KB.append(i)
    ...
    for t in range(50):
        ...
        # Add pacman location time step to KB
        timestep = [PropSymbolExpr(pacman_str, x, y, time=t) for x,y in non_wall_coords]
        KB.append(exactlyOne(timestep))

        # Add pacman direction time step to KB
        change = [PropSymbolExpr(action, time=t) for action in DIRECTIONS]
        KB.append(exactlyOne(change))

        # update KB based on Pacman movement
        if t == 0:
            KB.append(PropSymbolExpr(pacman_str, x0, y0, time=0))
        else:
            # possible locations for next step
            pacmanSuccessors = [pacmanSuccessorAxiomSingle(x, y, t, walls) for x, y in non_wall_coords]
            KB.extend(pacmanSuccessors)

            # possible food states for next step
            foodSuccessors = [foodSuccessorAxiom(x, y, t - 1) for x, y in food]
            KB.extend(foodSuccessors)
...
def localization(problem, agent) -> Generator:
    ...
    # iterate through all timesteps for agent
    for t in range(agent.num_timesteps):
        # create two sentences and add to KB
        ...
        # determine possible locations of agent
        possible_locations = [coord for coord in non_outer_wall_coords if 
                          entails(conjoin(KB), PropSymbolExpr(pacman_str, coord[0], coord[1], time=t))]
        
        for x, y in non_outer_wall_coords:
            # add PropSymbolExpr for agent location
        ...
        # add PropSymbolExpr to KB for action taken
...
def slam(problem, agent) -> Generator:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # add pacman loc to KB
    KB.append(PropSymbolExpr(pacman_str, pac_x_0,  pac_y_0, time=0))
    # add pacman loc negation to KB
    KB.append(~PropSymbolExpr(wall_str, pac_x_0, pac_y_0))
    # update map
    known_map[pac_x_0][pac_y_0] = 0
    ...
    for t in range(agent.num_timesteps):
        # add physics and percept rules to KB
        ...
        for x, y in non_outer_wall_coords:
            # pacman location
            px_expr = PropSymbolExpr(pacman_str, x, y, time=t)
            # wall location
            wp_expr = PropSymbolExpr(wall_str, x, y)
            # check if entailed
            if entails(conjoin(KB), px_expr):
                possible_locations.append((x, y))
                if px_expr not in KB:
                    KB.append(px_expr)
            # check negation
            elif entails(conjoin(KB), ~px_expr):
                if ~px_expr not in KB:
                    KB.append(~px_expr)
            else:
                possible_locations.append((x, y))

            if entails(conjoin(KB), wp_expr):
                known_map[x][y] = 1
                if wp_expr not in KB:
                    KB.append(wp_expr)
            elif entails(conjoin(KB), ~wp_expr):
                known_map[x][y] = 0
                if ~wp_expr not in KB:
                    KB.append(~wp_expr)
